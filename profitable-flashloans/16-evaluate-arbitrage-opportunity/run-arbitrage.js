...
const init = async () => {
  ...
  web3.eth.subscribe('newBlockHeaders')
    .on('data', async block => {
      ...
      const profit1 = (parseInt(AMOUNT_ETH_WEI) / 10 ** 18) * (kyberRates.sell - uniswapRates.buy) - (txCost / 10 ** 18) * currentEthPrice;
      const profit2 = (parseInt(AMOUNT_ETH_WEI) / 10 ** 18) * (uniswapRates.sell - kyberRates.buy) - (txCost / 10 ** 18) * currentEthPrice;
      if(profit1 > 0) {
        ...
        console.log(`Sell ETH from Uniswap at ${uniswapRates.sell} dai`);
